...
from material.models import Distributor
...
class Event(models.Model):
    ...
    equip             = models.TextField(blank=True, null=True, help_text='Equipment')
    details           = models.TextField(blank=True, null=True, help_text='Textual Notes')
    description       = models.TextField(_("description"), blank=True)
    start_time        = models.TimeField(blank=True, null=True, help_text='Meet at the office')
    start             = models.DateTimeField(_("start"), db_index=True)
    end               = models.DateTimeField(_("end"), db_index=True, help_text=_("The end time must be later than the start time."))
    dist_time         = models.TimeField(blank=True, null=True, help_text='Pickup parts')
    distributor       = models.ForeignKey(Distributor, on_delete=models.SET_NULL, blank=True, null=True, help_text='distributor')
    lead              = models.ForeignKey("munkz.ArtistProfile", on_delete=models.SET_NULL, blank=True, null=True, related_name='SchLead', help_text='Choose artists')
    artist            = models.ManyToManyField("munkz.ArtistProfile", blank=True, related_name='SchArtistProfile', help_text='Choose field ArtistProfile')
    ...
    visibility = models.CharField(
        max_length=10, choices=VISIBILITY_CHOICES, default='public', help_text="Visibility of the event"
    )
