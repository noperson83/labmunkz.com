...
from schedule.forms import EventForm, NewEventForm, OccurrenceForm
...
def EventCreateView(request, proj):
    form_class = NewEventForm
    psite = Project.objects.get(job_num=proj) 
    jobsi = psite.jobsite.id
    color = "#{:06x}".format(random.randint(0, 0xFFFFFF))
    template = 'schedule/event_form.html'
    if request.method == 'POST':
        ...
        form = NewEventForm(initial={"project": psite.id})
    return render(
        request,
        template,
        context={
...
class EventUpdate(UpdateView):
    model = Event
    fields = ['project', 'lead', 'aartist', 'text', 'equip', 'details', 'start_time', 'start', 'end', 'title', 'description', 'creator', 'rule', 'end_recurring_period', 'calendar', 'color_event']
