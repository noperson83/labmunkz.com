...
from schedule.views import (
    CalendarByPeriodsView, CalendarView, CancelOccurrenceView, CreateEventView, EventCreateView,
    ...
    api_move_or_resize_by_code, api_occurrences, api_select_create, EventTodayArchiveView, EventDayArchiveView,
)
app_name = 'schedule'
urlpatterns = [
    #re_path(r'^detail/schedule/(?P<pk>\d+)$', views.DayEventDetailView.as_view(), name='event-detail'),
    #path('create-event/', views.EventCreate.as_view(), name='create-event'), 
    url(r'create-event/(?P<proj>[0-9]+)$', views.EventCreateView, name='create-event'), 
    #url(r'schedule/update-event/(?P<pk>[0-9]+)/$', views.DayEventUpdate.as_view(), name='update-event'),
    #url(r'schedule/event/(?P<pk>[0-9]+)/delete/$', views.DayEventDelete.as_view(), name='delete-event'),
    path('today/', EventTodayArchiveView.as_view(), name='archive_today'),
    path('<int:year>/<str:month>/<int:day>/', EventDayArchiveView.as_view(), name='archive_day'),
